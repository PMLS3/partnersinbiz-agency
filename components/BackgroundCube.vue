<template>
  ...
  >
    <perspective-camera
      ...
      :rotation="rotation"
    ></perspective-camera>
    <scene slot="scene" ref="scene">
      <mesh ref="cube" id="cube" slot="object">
        <box-geometry
          slot="geometry"
          :width="0.5"
          :height="0.5"
...
export default {
  ...
  methods: {
    ...
    update() {
      ...
      if (
        this.$refs.cube.object.position.x >= 2 ||
        this.$refs.cube.object.position.x <= -2
      ) {
        this.sign.x *= -1
      ...
      if (
        this.$refs.cube.object.position.y >= 2 ||
        this.$refs.cube.object.position.y <= -2
      ) {
        this.sign.y *= -1
  ...
  mounted() {
    ...
    let cubes = this.$refs.cube
    let sc = this.$refs.scene
