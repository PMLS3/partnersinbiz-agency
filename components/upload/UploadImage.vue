    ...
    <vs-popup fullscreen :title="label" :active.sync="popupActivo4">
      ...
      >
        ...
        <vs-button
          ...
          icon="add_circle"
          style="float: right"
          v-if="!needUpload && selectedImages.length > 0"
          >Use Selected</vs-button
        >
        <div v-if="!needUpload">
          <vs-tabs color="success" v-if="categories.length > 0">
            <vs-tab
              @click="catSelect = `${item.category}`"
              :label="item.category"
        ...
        <div class="flex items-center mt-8">
          ...
          <p class="px-2" v-if="needUpload">or</p>
          <vs-input
            v-model="categorySave"
            v-if="needUpload"
        ...
        <div class="w-full mt-5" v-if="needUpload">
          <div style="outline: none">
            <div
              class="text-center image-container position-relative"
              v-if="!images.length"
            >
              <div
            ...
            <div
              class="text-center image-container position-relative image-list"
              v-else
            >
              <div
                class="cursor-pointer preview-image full-width position-relative"
              ...
              >
                ...
                <div class="display-flex">
                  <label
                    class="cursor-pointer image-edit display-flex"
                    :for="idEdit"
                  >
                    <svg
                  ...
                  <a
                    class="cursor-pointer image-delete display-flex"
            ...
            <div
              class="flex-wrap image-list-container display-flex"
              v-if="images.length && multiple"
            >
              <div
                class="cursor-pointer image-list-item position-relative"
              ...
              <div
                class="cursor-pointer image-list-item position-relative display-flex justify-content-center align-items-center"
...
export default {
  ...
  data() {
    return {
      ...
      needUpload: false,
      selectedCategory: '',
      categories: [{ id: 1, category: 'Default' }],
      selectedImages: [],
      catSelect: '',
      categoryImages: [],
    }
  ...
  methods: {
    ...
    setImagesNew() {
      ...
      for (let i = 0; i < this.infoImages.length; i++) {
        if (this.infoImages[i].category == this.catSelect) {
          this.categoryImages.push(this.infoImages[i])
        }
      }
    },
    onSelectMultipleImage(i) {
      let myImages = []

      for (let x = 0; x < i.length; x++) {
        myImages.push(i[x].downloadURL)
      }

      this.selectedImages = myImages
    },
    emitChanges() {
      let vm = this
      this.submitStore()
    ...
    submitStore() {
      this.$store.commit('form/UPLOADED_IMAGES', this.selectedImages)
    },

    AddCategory() {
      let addingInformationLocation = this.$fireStore.collection('category')
      let vm = this
      addingInformationLocation
        .add({
        ...
        .then(function (docRef) {
          ...
          vm.successUpload('Category loaded')
        })
    ...
    setImages() {
      ...
      let addingInformationLocation = this.$fireStore.collection('images')
      ...
      for (let i = 0; i < this.images.length; i++) {
        const mountainsRef = storageRef.child(
          `${this.business.b_uid}/${this.user.uid}/${this.images[i].name}`
        )

        mountainsRef
          .put(this.images[i].file)
          .then((snapshot) => {
            snapshot.ref.getDownloadURL().then((downloadURL) => {
              settingImages.push(downloadURL)
              ...
              addingInformationLocation.add({
                ...
                category: category,
                alt: this.images[i].name,
                path: `${this.business.b_uid}/${this.user.uid}/${this.images[i].name}`,
              })
            })
          })
          .then(function (docRef) {
            vm.successUpload(
              'Image loaded ' + `${i + 1}` + ' of ' + vm.images.length
            )
          })
          .catch(function (error) {
            console.error('Error adding document: ', error)
  ...
  watch: {
    ...
    catSelect() {
      this.setImagesNew(this.catSelect)
    },
  },
  mounted() {
    ;[
  ...
  created() {
    if (process.client) {
      ...
      let infoRetrieve = this.$fireStore.collection('images')
