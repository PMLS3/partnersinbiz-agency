...
<template>
  <div id="faq-page" @click="addComponent('faq-page')">
    <!-- JUMBOTRON -->
    <div class="faq-jumbotron">
      <div
        class="p-8 rounded-lg faq-jumbotron-content lg:p-32 md:p-24 sm:p-16 mb-base"
      >
        <h1 class="mb-1 text-white">Have Any Questions?</h1>
        <p class="text-white">
          Bonbon sesame snaps lemon drops marshmallow ice cream carrot cake
          croissant wafer.
        </p>
        <vs-input
          placeholder="Search"
    ...
    <div class="vx-row">
      <div class="w-full rounded-lg vx-col md:w-2/5 lg:w-1/4">
        <vx-card>
          <h4>Table of Content</h4>
          <ul class="mt-4 faq-topics">
            <li
              v-for="category in categories"
              :key="category.id"
              class="flex items-center p-2 font-medium"
      ...
      <!-- FAQ COL -->
      <div class="w-full mt-12 vx-col md:w-3/5 lg:w-3/4 md:mt-0">
        <vs-collapse accordion type="margin" class="p-0">
          <vs-collapse-item
            v-for="(que, index) in filteredFaq"
            class="rounded-lg faq-bg"
            :class="{ 'mt-0': !index }"
            :key="que.id"
          >
            <div slot="header">
