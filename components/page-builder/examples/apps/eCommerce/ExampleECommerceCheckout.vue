...
const ItemListView = () => import('./components/ExampleItemListView.vue')
...
export default {
  ...
  computed: {
    cartItems() {
      return this.$store.state.example.cartItems.slice().reverse()
    },
    isInWishList() {
  ...
  methods: {
    ...
    removeItemFromCart(item) {
      this.$store.dispatch('example/toggleItemInCart', item)
    },
    ...
    toggleItemInWishList(item) {
      this.$store.dispatch('example/toggleItemInWishList', item)
    },
    updateItemQuantity(event, index) {
      const itemIndex = Math.abs(index + 1 - this.cartItems.length)
      this.$store.dispatch('example/updateItemQuantity', {
        quantity: event,
        index: itemIndex,
      })
    },
