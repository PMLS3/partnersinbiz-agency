...
export default {
  ...
  computed: {
    chatData() {
      return this.$store.getters['example/chatDataOfUser'](this.userId)
    },
    activeUserImg() {
      return this.$store.state.auth.active_user.avatart
    },
    senderImg() {
      return isSentByActiveUser => {
        if (isSentByActiveUser) return this.$store.state.auth.active_user.avatar
        else return this.$store.getters['example/contact'](this.userId).avatar
      }
    },
    hasSentPreviousMsg() {
