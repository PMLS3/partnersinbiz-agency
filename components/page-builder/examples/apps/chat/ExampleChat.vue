...
<script>
import ChatContact from './ExampleChatContact.vue'
import ChatLog from './ExampleChatLog.vue'
import ChatNavbar from './ExampleChatNavbar.vue'
import UserProfile from './ExampleUserProfile.vue'
...
export default {
  ...
  computed: {
    chatLastMessaged() {
      return (userId) => this.$store.getters['example/chatLastMessaged'](userId)
    },
    chatUnseenMessages() {
      return (userId) => {
        const unseenMsg = this.$store.getters['example/chatUnseenMessages'](
          userId
        )
    ...
    chatContacts() {
      return this.$store.getters['example/chatContacts']
    },
    contacts() {
      return this.$store.getters['example/contacts']
    },
    searchQuery: {
      get() {
        return this.$store.state.example.chatSearchQuery
      },
      set(val) {
        this.$store.dispatch('example/setChatSearchQuery', val)
      },
    },
    scrollbarTag() {
  ...
  methods: {
    getUserStatus(isActiveUser) {
      // return "active"
      return isActiveUser
        ? this.$store.state.auth.active_user.status
    ...
    updateActiveChatUser(contactId) {
      ...
      if (this.$store.getters['example/chatDataOfUser'](this.activeChatUser)) {
        this.$store.dispatch('example/markSeenAllMessages', contactId)
      }
      const chatData = this.$store.getters['example/chatDataOfUser'](
        this.activeChatUser
      )
    ...
    sendMsg() {
      ...
      this.$store.dispatch('example/sendChatMessage', payload)
      this.typedMessage = ''
  ...
  created() {
    this.$store.dispatch('example/fetchContacts')
    this.$store.dispatch('example/fetchChatContacts')
    this.$store.dispatch('example/fetchChats')
    this.setSidebarWidth()
  },

  mounted() {
    this.$store.dispatch('example/setChatSearchQuery', '')
  },
}
</script>
