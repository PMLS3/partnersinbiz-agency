<template>
  <div>
    ...
    <!-- <div v-for="img in placeholder" :key="img" v-if="edit">
<img :src="img" style="width: 40%" />
        </div> -->
    <!-- SINGLE SELECTION -->
    <vs-button @click="popupActivo4 = true" color="primary" icon="photo_camera">
      Upload</vs-button
    >
    <vs-popup
      fullscreen
    ...
    >
      <vs-button
        @click="needUpload = true"
        color="success"
        type="border"
        style="float: right;"
        >Upload More</vs-button
      >
      ...
      <div v-for="(audio, index) in infoAudios" :key="index">
        <vs-checkbox v-model="infoAudios[index].selected">{{
          audio.name
        }}</vs-checkbox>
        <audio id="audio" controls>
          <source :src="audio.src" id="src" />
        </audio>
      </div>

      <vs-input v-model="categorySave" label="Add Category" v-if="needUpload" />
      <vs-button @click="AddCategory" v-if="needUpload">Add Category</vs-button>
      ...
      <div class="w-full mt-5" v-if="needUpload">
        <div style="outline: none;">
          ...
          >
            <div
              ...
              @drop="onDrop"
            >
              <div class="text-center centered full-width text-primary">
                <svg
            ...
            <div
              v-else
              class="audio-center position-absolute display-flex flex-column justify-content-center align-items-center"
              @dragover.prevent="onDragover"
            >
              <div>
                <svg
                  class="audio-icon-drag"
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 512 512"
                >
                  <path
              ...
              <div class="audio-input position-absolute full-width full-height">
                <label
                  :for="idUpload"
          ...
          <div
            class="text-center audio-container position-relative audio-list"
            v-else
          >
            <div
              class="cursor-pointer preview-audio full-width position-relative"
              @click="openGallery(currentIndexAudio)"
            >
              <div
                class="audio-overlay position-relative full-width full-height"
              ></div>
              <div class="audio-overlay-details full-width">
                <svg
                  class="icon-overlay"
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 512 512"
              ...
              <div class="show-audio centered">
                <img class="show-img img-responsive" :src="audioPreview" />
                <p>here</p>
              </div>
            </div>
            <div
              class="audio-bottom display-flex position-absolute full-width align-items-center justify-content-between"
              :class="!showPrimary && 'justify-content-end'"
            >
              <div
                class="audio-bottom-left display-flex align-items-center"
                v-if="showPrimary"
              >
                <div
                  ...
                  v-show="audioDefault"
                >
                  <span class="audio-primary display-flex align-items-center">
                    <svg
                      class="audio-icon-primary"
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 512 512"
                    >
                      <circle fill="#10BC83" cx="256" cy="256" r="256" />
                      <path
                        fill="#FFF"
                ...
                <a
                  class="cursor-pointer text-small mark-text-primary"
                  @click.prevent="markIsPrimary(currentIndexAudio)"
                  v-show="!audioDefault"
                  >{{ markIsPrimaryText }}</a
                >
              </div>
              <div class="display-flex">
                <label
                  class="cursor-pointer audio-edit display-flex"
                  :for="idEdit"
                >
                  <svg
                    class="audio-icon-edit"
                    xmlns="http://www.w3.org/2000/svg"
                    width="512"
                    height="512"
                    viewBox="0 0 512 512"
                ...
                <a
                  class="cursor-pointer audio-delete display-flex"
                  @click.prevent="deleteAudio(currentIndexAudio)"
                >
                  <svg
                    class="audio-icon-delete"
                    xmlns="http://www.w3.org/2000/svg"
                    width="512"
                    height="512"
                    viewBox="0 0 512 512"
          ...
          <div
            class="flex-wrap audio-list-container display-flex"
            v-if="audios.length && multiple"
          >
            <div
              class="cursor-pointer audio-list-item position-relative"
              :class="audio.highlight && 'audio-highlight'"
              v-for="(audio, index) in audios"
              :key="index"
              @click="changeHighlight(index)"
            >
              <div class="centered">
                <!-- <img class="show-img img-responsive" :src="audio.path" />
                 -->
                <small>{{ audio.name }}</small>
                <!-- <audio id="audio" controls>
                  <source :src="audio.path" id="src" />
                </audio> -->
              </div>
            </div>
            <div
              class="cursor-pointer audio-list-item position-relative display-flex justify-content-center align-items-center"
              v-if="audios.length < maxAudio"
            >
              <svg
                class="icon add-audio-svg"
                xmlns="http://www.w3.org/2000/svg"
                width="512"
                height="512"
                viewBox="0 0 512 512"
              ...
              <div
                class="input-add-audio position-absolute full-width full-height"
              >
                <label
                  :for="idUpload"
                  class="cursor-pointer display-block full-width full-height"
                ></label>
              </div>
            </div>
          </div>
        ...
        <vs-button @click="setAudios" class="my-6">Use Audio</vs-button>
      </div>
    </vs-popup>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    emitChanges() {
      ...
      this.$emit('input', this.selectedAudios)
      ;(this.popupActivo4 = false), (this.needUpload = false)
    },

    AddCategory() {
      let addingInformationLocation = this.$fireStore
        .collection('user')
        .doc('music')
        .collection('music')
        .doc('category')
      ...
      for (let i = 0; i < this.audios.length; i++) {
        ...
        mountainsRef.put(this.audios[i].file).then(snapshot => {
          snapshot.ref.getDownloadURL().then(src => {
            settingAudios.push(src)
            this.audioUrls = src
            addingInformationLocation.add({
              src,
              category: this.selectedCategory.category,
              name: this.audios[i].name,
              img: ''
            })
          })
        })
...
.preview-audio:hover .audio-overlay,
.preview-audio:hover .audio-overlay-details {
  opacity: 1;
}
.img-responsive {
  display: block;
...
.audio-list-container .audio-list-item {
  height: 32px;
  width: 32px;
  border-radius: 4px;
  border: 1px solid #d6d6d6;
}
.audio-list-container .audio-list-item:not(:last-child) {
  margin-right: 5px;
  margin-bottom: 5px;
}
.audio-list-container .audio-list-item .show-img {
  max-width: 25px;
  max-height: 17px;
}
.audio-list-container .audio-highlight {
