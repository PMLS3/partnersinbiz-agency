...
<template>
  <client-only>
    <div id="ag-grid-demo">
      ...
      >
        <div class="px-6 pb-12">
          <p>Please fill in the fields you want to update</p>
          <FormGenerator :schema="schema" v-model="formData" />
          <vs-button @click="updateForm" class="mt-8">Submit</vs-button>
        </div>
      </vs-popup>
    </div>
  </client-only>
...
export default {
  ...
  computed: {
    ...
    edit_form() {
      return this.$store.state.form.edit_form
    },
    totalPages() {
  ...
  watch: {
    edit_form: function () {
      this.editForm = true
      let newForm = this.schema
      let object = this.edit_form
      console.log('object')
      console.log('sche', newForm)
