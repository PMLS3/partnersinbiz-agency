<!-- =========================================================================================
    File Name: Pagination.vue
    Description: Pagination - Imports all page portions
    ----------------------------------------------------------------------------------------
  
========================================================================================== -->

<template>
  <div id="pagination-demo">
    <pagination-default></pagination-default>
    <pagination-icon></pagination-icon>
    <pagination-links-limit></pagination-links-limit>
    <pagination-go-to></pagination-go-to>
    <pagination-color></pagination-color>
    <pagination-programmatic-inc-dec></pagination-programmatic-inc-dec>
  </div>
</template>

<script>
import PaginationDefault from './AppsViewsPaginationDefault.vue'
import PaginationIcon from './AppsViewsPaginationIcon.vue'
import PaginationLinksLimit from './AppsViewsPaginationLinksLimit.vue'
import PaginationGoTo from './AppsViewsPaginationGoTo.vue'
import PaginationColor from './AppsViewsPaginationColor.vue'
import PaginationProgrammaticIncDec from './AppsViewsPaginationProgrammaticIncDec.vue'

export default {
  components: {
    PaginationDefault,
    PaginationIcon,
    PaginationLinksLimit,
    PaginationGoTo,
    PaginationProgrammaticIncDec,
    PaginationColor,
  },
}
</script>
