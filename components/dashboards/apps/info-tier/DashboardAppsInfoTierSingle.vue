            ...
            <div class="flex flex-row">
              <vs-tooltip text="View Component" position="top">
                <vs-button
                  class="ml-1"
                  icon="preview"
                  @click="$router.push(`/AppsInfoTier/${$route.params.id}`)"
                ></vs-button>
              </vs-tooltip>
...
export default {
  ...
  data() {
    return {
      item: { item: 'InfoSingle', title: 'Load Images', type: 'Single' },
  ...
  computed: {
    ...
    userUrl() {
      var url =
        window.location.origin + '/se/AppsInfoTier/' + this.$route.params.id
