            ...
            <div class="flex flex-row">
              <vs-tooltip text="View Component" position="top">
                <vs-button
                  class="ml-1"
                  icon="preview"
                  @click="$router.push(`/AppsImageGallery/${$route.params.id}`)"
                ></vs-button>
              </vs-tooltip>
...
export default {
  ...
  data() {
    return {
      item: { item: 'Images', title: 'Load Images', type: 'Single' },
  ...
  computed: {
    ...
    userUrl() {
      var url =
        window.location.origin + '/se/AppsImageGallery/' + this.$route.params.id
