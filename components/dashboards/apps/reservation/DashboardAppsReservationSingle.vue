            ...
            <div class="flex flex-row">
              <vs-tooltip text="View Component" position="top">
                <vs-button
                  class="ml-1"
                  icon="preview"
                  @click="$router.push(`/AppsReservation/${$route.params.id}`)"
                ></vs-button>
              </vs-tooltip>
...
export default {
  ...
  data() {
    return {
      item: { item: 'ReservationSingle', title: 'Load Images', type: 'Single' },
  ...
  computed: {
    ...
    userUrl() {
      var url =
        window.location.origin + '/se/AppsReservation/' + this.$route.params.id
    ...
    schema() {
      return [
        ...
        {
          title: 'NumberInput',
          placeholder: 'How many can be served at once',
          name: 'availability',
          label: 'Availability',
          minValue: 0,
        },
