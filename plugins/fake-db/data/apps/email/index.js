import mock from '~/plugins/fake-db/mock.js'
...
const data = {
  emails: [
    ...
    },
    {
      ...
      replies: [
        ...
        },
        {
          ...
          bcc: [],
          message:
            '<p>Oat cake tart danish jelly beans brownie I love. Liquorice I love lollipop chocolate cake carrot cake toffee. Tart muffin candy canes croissant sugar plum lollipop. Macaroon cheesecake marshmallow powder sweet roll bonbon candy apple pie candy canes.</p><p><br></p><p>Regrads,</p><p>Hettie Mcerlean</p><p><br></p>',
    ...
    },
    {
      ...
      bcc: [],
      message:
        '<p>Hello John, </p><p><br></p><p>unstintedly septifarious prevalid implicative esere Kedarite Nekkar encampment Issedones Saururus Turkize Tshi misengrave lamnid benzamine rigidulous octonion hydrophilous coxalgic nasturtium epileptogenic biporose undenominationalism widu</p><p><br></p><p>melioration pager infiltrate plouky tribesmanship transudate heinous exhibitionistic klendusic lienocele tractate unemotioned castigate selenate appealable Palladianism chloracetate penna moellon detector halcyon amidofluorid damascene hollaite</p>',
    ...
    },
    {
      ...
      bcc: [],
      message:
        '<p>Hello John, </p><p><br></p><p>nonmotile generale punctated humanify escharine unlatch alrighty unshepherding standstill upcrane unilobular prefrontal primate fluviatic premundane tritocone compaternity dolina phonautographic cobby corse neurohypnotic cyrtopia wreathy</p><p><br></p><p>slanderousness Lemosi myxochondroma haemogram electrothermostat piperonyl spital reappeal pyopneumothorax Yunca eyoty forecourse extensive kickout germanization vigilant Criophoros muss aortostenosis Berchta phonophotoscopic precorrespondence rigidist castling</p>',
      attachments: ['Hac.xls'],
      isStarred: true,
      labels: ['personal'],
  ...
  data.emails.forEach(mail => {
    ...
    if (mail.unread) {
      ...
      if (countUnreadMailFolders.includes(mail.mailFolder)) {
        meta.unreadMails.folder[mail.mailFolder]
          ? meta.unreadMails.folder[mail.mailFolder].push(mail.id)
          : (meta.unreadMails.folder[mail.mailFolder] = [mail.id])
      }

      // Add mail id
      mail.labels.forEach(label => {
        meta.unreadMails.label[label]
          ? meta.unreadMails.label[label].push(mail.id)
          : (meta.unreadMails.label[label] = [mail.id])
      })
    }
...
mock.onPost('/api/apps/email/move-mails').reply(request => {
  ...
  data.emails.forEach(mail => {
    if (mailsToMove.includes(mail.id))
      mail.mailFolder = JSON.parse(request.data).mailFolder
  })
...
mock.onPost('/api/apps/email/set-labels').reply(request => {
  ...
  data.emails.find(mail => mail.id === mailId).labels = JSON.parse(
    request.data
  ).labels
  return [200]
})
...
mock.onPost('/api/apps/email/set-starred').reply(request => {
  ...
  data.emails.find(mail => mail.id === mailId).isStarred = JSON.parse(
    request.data
  ).value
