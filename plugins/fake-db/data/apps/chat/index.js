...
mock.onGet('/api/apps/chat/contacts').reply(request => {
  ...
  const filteredContacts = data.contacts.filter(contact => {
    return contact.displayName
      .toLowerCase()
      .includes(request.params.q.toLowerCase())
  })
...
mock.onGet('/api/apps/chat/chat-contacts').reply(request => {
  const chatContactsArray = data.contacts.filter(contact => {
    if (data.chats[contact.uid])
      return (
        data.chats[contact.uid] &&
        contact.displayName
          .toLowerCase()
          .includes(request.params.q.toLowerCase())
      )
...
mock.onPost('/api/apps/chat/msg').reply(request => {
  ...
  if (payload.chatData) {
    ...
    data.chats[Object.keys(data.chats).find(key => key == payload.id)].msg.push(
      payload.msg
    )
  } else {
