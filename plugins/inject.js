...
export default (context, inject) => {
  ...
  const comp_create = (element) => {
    ...
    for (let i = 0; i < child_element_count; i++) {
      item.children.push(childElementLoop(children[i]))
    }

    payload.push(item)

    function childElementLoop(elm) {
      ...
      for (let i = 0; i < child_element_count; i++) {
        child_item.children.push(childElementLoop(children[i]))
      }
      return child_item
    }
