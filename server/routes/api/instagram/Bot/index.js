class InstagramBot {
  ...
  async visitInstagram() {
    ...
    // Close or Turn off Save Your login details
    if (this.config.selectors.not_now_button_save) {
      await this.page.click(this.config.selectors.not_now_button_save)
    }
    await this.page.waitForNavigation()
  ...
  async _doPostLikeAndFollow(parentClass, page) {
    for (let r = 1; r < 4; r++) {
      ...
      for (let c = 1; c < 4; c++) {
        ...
        console.log('empty', hasEmptyHeart)
        ...
        if (
          ...
          console.log('liking button', this.config.selectors.post_like_button)
  ...
  async unFollowUsers() {
    let date_range =
      new Date().getTime() - this.config.settings.unfollow_after_days * 86400000
